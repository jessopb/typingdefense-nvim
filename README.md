...
As you type, correct characters and mistakes are highlighted live, `<BS>`
lets you fix mistakes, and finishing the text shows WPM, accuracy, elapsed
time, and mistake count. `<Esc>`/`<C-c>` aborts a test in progress.
...
```lua
{
  ...
  cmd = { "TypingWords", "TypingLesson", "TypingStop" },
  opts = {},
}
```
...
:TypingStop              " abort the current test
```
